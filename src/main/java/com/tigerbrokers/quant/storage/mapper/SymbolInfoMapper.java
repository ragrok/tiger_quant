...
public interface SymbolInfoMapper {
    ...
    @Insert({"<script> insert into t_symbol_info(f_contract_id,f_identifier,f_symbol,f_sec_type," +
            ...
            "f_exchange,f_market,f_primary_exchange,f_currency,f_local_symbol,f_name,f_tiger_status,f_min_tick,f_trade,f_continuous,f_trading_class) value " +
            "(#{contractId},#{identifier},#{symbol},#{secType}," +
            ...
            "#{exchange},#{market},#{primaryExchange},#{currency},#{localSymbol},#{name},#{status},#{minTick},#{trade},#{continuous},#{tradingClass}) </script>"})
